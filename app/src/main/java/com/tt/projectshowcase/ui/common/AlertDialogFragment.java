 ...
 *         \date 20.9.2017.
 *
 *         Wrapper class with various builder options to constructor a custom DialogFragment without the need to always subclass it.
 */
public class AlertDialogFragment extends DialogFragment implements View.OnClickListener {
    ...
    private static final String KEY_TEXT_VIEWS = "textViews";
    ...
    private static AlertDialogFragment newInstance(Builder b){
        ...
        args.putParcelableArray(KEY_TEXT_VIEWS, b.customViewTexts);
    ...
    public void onAttach(Context context) {
        ...
        if(context instanceof AlertDialogClickInterface){
            clickInterface = (AlertDialogClickInterface) context;
        }
    }
                ...
                if(clickableViews != null){
                    for (ClickableView clickableView : clickableViews) {
                        ...
                        if (childView != null) {
                            ...
                            Log.w(TAG, "Child not found.");
                        }
                    }
                }

                CustomViewText[] customViewTexts = (CustomViewText[]) bundle.getParcelableArray(KEY_TEXT_VIEWS);
                if(customViewTexts != null){
                    for (CustomViewText tv : customViewTexts) {
                        android.widget.TextView textView = v.findViewById(tv.viewId);
                        if(textView != null){
                            if(tv.text != null) {
                                textView.setText(tv.text);
                            } else if(tv.textId != 0){
                                textView.setText(tv.textId);
                            }
                        } else {
                            Log.w(TAG, "Child not found.");
                        }
    ...
    public static class Builder {
        ...
        private CustomViewText[] customViewTexts;
        ...
        public Builder withTextViews(CustomViewText... customViewTexts){
            this.customViewTexts = customViewTexts;
            return this;
        }
    ...
    public static class CustomViewText implements Parcelable {

        private final int viewId;
        private final String text;
        private final int textId;

        public CustomViewText(@IdRes int viewId, String text) {
            this.viewId = viewId;
            this.text = text;
            this.textId = 0;
        }

        public CustomViewText(@IdRes int viewId, @StringRes int textResId){
            this.viewId = viewId;
            this.textId = textResId;
            this.text = null;
        }

        protected CustomViewText(Parcel in) {
            viewId = in.readInt();
            text = in.readString();
            textId = in.readInt();
        }
        ...
        public static final Creator<CustomViewText> CREATOR = new Creator<CustomViewText>() {
            @Override
            public CustomViewText createFromParcel(Parcel in) {
                return new CustomViewText(in);
            }

            @Override
            public CustomViewText[] newArray(int size) {
                return new CustomViewText[size];
            }
        };
    }
